...
if (!supabaseUrl || !supabaseKey) {
  console.error('Usage: node import-broker-reviews.js <SUPABASE_URL> <SUPABASE_KEY>');
  process.exit(1);
}
  ...
  try {
    ...
    for (const broker of brokers) {
      ...
      for (let i = 0; i < reviewCount; i++) {
        ...
        const reviewText = generateReviewText(broker.name, rating, broker.supported_assets);
        ...
        try {
          ...
          const { data: review, error } = await supabase
            ...
            .insert({
              ...
              reviewer_name: reviewerName // Add reviewer's name to the comment
            })
            .select();
