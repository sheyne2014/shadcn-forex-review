// Import Broker Reviews Script (Integer Ratings Version)
...
if (!supabaseUrl || !supabaseKey) {
  console.error('Usage: node import-broker-reviews-int-rating.js <SUPABASE_URL> <SUPABASE_KEY>');
  process.exit(1);
}
...
// Random rating function (provides an integer rating close to the broker's overall rating)
...
async function importBrokerReviews() {
  console.log('🚀 Starting import of broker reviews (integer ratings version)...');
  ...
  try {
    ...
    // Check the reviews table schema to confirm the data type of rating
    ...
    try {
      const { data: columns, error } = await supabase
        ...
        .eq('column_name', 'rating');
      
      if (error) {
        console.warn('⚠️ Warning: Could not check reviews table schema:', error.message);
      } else if (columns && columns.length > 0) {
        console.log(`Rating column data type: ${columns[0].data_type}`);
      }
    } catch (schemaError) {
      console.warn('⚠️ Warning: Error checking schema:', schemaError.message);
    }
    ...
    for (const broker of brokers) {
      try {
        ...
        for (let i = 0; i < reviewCount; i++) {
          ...
          // Generate an INTEGER rating - should be close to broker's overall rating
          ...
          try {
            console.log(`Adding review for ${broker.name} with rating: ${rating} (integer)`);
            ...
            const { data: review, error: reviewError } = await supabase
              ...
              .insert({
                ...
                rating: rating, // This is now an integer
                comment: reviewText,
              ...
              try {
                const { data: retryReview, error: retryError } = await supabase
                  ...
                  .insert({
                    broker_id: broker.id,
                    user_id: randomUserId,
                    rating: parseInt(rating, 10), // Explicitly convert to integer
                    comment: reviewText,
                    created_at: createdAt
                  })
                  .select();
                ...
                if (retryError) {
                  ...
                  totalReviewsAdded++;
                  brokerReviewsAdded++;
                  console.log(`✅ Retry successful for ${broker.name} review`);
                }
              } catch (retryException) {
                console.error(`❌ Exception during retry: ${retryException.message}`);
              }
