...
import { getBrokerBySlug, getSimilarBrokers, getMockBroker } from "@/lib/brokers";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  // Get the broker data using the slug
  const broker = getMockBroker('interactive-brokers');

  // Customize the broker data for Interactive Brokers
  broker.name = "Interactive Brokers";
  broker.description = "Interactive Brokers is a global electronic brokerage firm offering trading in stocks, options, futures, currencies, bonds, and funds.";
  broker.logo_url = "/images/brokers/interactive-brokers.png";
  broker.website_url = "https://www.interactivebrokers.com";
  broker.min_deposit = 0;
  broker.max_leverage = "1:4";
  broker.regulations = "SEC, FINRA, FCA, CFTC";
  broker.trading_platforms = "IBKR Trader Workstation, IBKR Mobile, Client Portal";
  broker.spreads_from = "0.1 pips";
  broker.account_types = ["Cash", "Margin", "Portfolio Margin", "IRA"];
  broker.country = "United States";
  broker.established = "1978";
  broker.overall_rating = 4.8;
  ...
  return {
    ...
    openGraph: {
      ...
      description: seoData.description,
      type: "article",
      url: `https://yoursite.com/brokers/interactive-brokers`,
      images: [
        {
...
export default async function BrokerReviewPage({ params }: Props) {
  // Get the broker data using the mock function
  const broker = getMockBroker('interactive-brokers');
  ...
  broker.established = "1978";
  broker.overall_rating = 4.8;
  broker.pros = ["Extensive product range", "Advanced trading platforms", "Low commissions", "Global market access"];
  broker.cons = ["Complex platform for beginners", "High account minimums for some features", "Customer service can be slow"];
  broker.educational_resources = true;
