...
// List of broker slugs to update
const brokerSlugs = [
  'axi',
  'swissquote',
  'startrader',
...
// Template for the broker page
const generateBrokerPageContent = (slug) => `import { Metadata } from "next";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const broker = getMockBroker('${slug}');

  // Apply template data to broker
  if (brokerTemplate) {
...
// Update each broker page
brokerSlugs.forEach(slug => {
  // Create directory if it doesn't exist
  const dirPath = path.join('src', 'app', 'brokers', slug);
  ...
  fs.writeFileSync(filePath, generateBrokerPageContent(slug));
  console.log(`Updated broker page: ${filePath}`);
});

console.log('All broker pages have been updated successfully!');
